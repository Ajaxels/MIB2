...
% this menu allows to send the the currently opened image to Fiji after press of the [class.kbd]Export[/class] button (*4*). 
...
%% 5. The Import button
% The [class.kbd]Import[/class] button allows to transfer datasets from Fiji to
...
% is shown in the edit box by default. The command [class.code] run('Flip Z') [/class] will Flip the Z-dimension of the selected in Fiji dataset.
% The execution of the macro is called by pressing the [class.kbd]Run[/class] button (*8*).
%
...
% into this field. The file can be also selected using the [class.kbd]Select file...[/class] button (*7*)
...
% execution of the script is done by pressing the [class.kbd]Run[/class] button (*8*)
%% 8. The Run button
% Execute a macro written in the |Run macro| edit box (*6*). When edit box has a path to a text file with a set of macros the
% [class.kbd]Run[/class] button loads that file and executes all the macros written in that script file.
%% 9. The Help button
...
%
% [dtls][smry] *Example* [/smry]
%
% # Open a dataset for tests
% # Press the [class.kbd]Start Fiji[/class] button to open an instance of Fiji
% # If you want to send the opened image to Fiji, select |image| in the |Image Type| combo box and press the [class.kbd]Export[/class] button
...
% # In |MIB| select |Fiji Connect->Image Type->mask| and press the [class.kbd]Import[/class] button
