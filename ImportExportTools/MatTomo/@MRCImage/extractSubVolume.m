% extractSubVolume  Extract a sub-volume from MRCImage
%   The subvolume at the closest whole voxel coordinate will be returned,
%   in additon to the fractional shift (in voxels) from the actual to the
%   requested position.
%
%   [subVolume shift] = extractSubVolume(mRCImage, center, volSize,    ...
%                                        test, meanFill, permissive)
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [subVolume, shift] =                                          ...
  extractSubVolume(mRCImage, center, volSize, test, meanFill, permissive)
...
if mRCImage.header.spaceGroup == 401
  PEETError('A particle stack can not be read as a single volume!');
end
...
[fullyInRange, partiallyInRange] =                                     ...
  checkIndexRange('X', szFullVol(1), rngX, test, meanFill, permissive);
  [full, part] =                                                       ...
    checkIndexRange('Y', szFullVol(2), rngY, test, meanFill, permissive);
  fullyInRange = fullyInRange && full;
  partiallyInRange = partiallyInRange && part;
  [full, part] =                                                       ...
    checkIndexRange('Z', szFullVol(3), rngZ, test, meanFill, permissive);
  fullyInRange = fullyInRange && full;
  partiallyInRange = partiallyInRange && part;
