...
defAns = {[num2str(obj.preferences.CLAHE.NumTiles(1)) ',' num2str(obj.preferences.CLAHE.NumTiles(2))],...
           num2str(obj.preferences.CLAHE.ClipLimit),...
           num2str(obj.preferences.CLAHE.NBins),...
           {'uniform', 'rayleigh', 'exponential', find(ismember({'uniform', 'rayleigh', 'exponential'}, obj.preferences.CLAHE.Distribution)==1)}, ...
           num2str(obj.preferences.CLAHE.Alpha)};
mibInputMultiDlgOpt.PromptLines = [1, 3, 5, 1, 2];
mibInputMultiDlgOpt.Title = sprintf('You are going to change image contrast by Contrast-limited adaptive histogram equalization for color channel:%d\nYou can always undo it with Ctrl-Z', colCh);
mibInputMultiDlgOpt.TitleLines = 4;

answer = mibInputMultiDlg([], prompt, defAns, 'Enter CLAHE parameters', mibInputMultiDlgOpt);
