...
if obj.mibModel.preferences.max3dUndoHistory <= 1 && storeOptions.switch3d  % tweak for storing a single 3D dataset
    ...
    if storeOptions.switch3d    % 3D case
        ...
        elseif strcmp(type, 'labels')
            [labels.labelText, labels.labelPosition] = obj.mibModel.I{obj.mibModel.Id}.hLabels.getLabels();
...
if storeOptions.switch3d     % 3D case
    for cellId = 1:numel(data)
        ...
        switch type
            ...
            case 'labels'
                data = data{cellId};
                obj.mibModel.I{obj.mibModel.Id}.hLabels.replaceLabels(data.labelText, data.labelPosition);
