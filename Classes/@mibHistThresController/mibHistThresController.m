classdef mibHistThresController < handle
    ...
    methods
        ...
        function updateWidgets(obj)
            ...
            obj.View.handles.ColorChannel.Value = obj.mibModel.I{obj.mibModel.Id}.selectedColorChannel;
            
            obj.View.handles.ColorChannel.String = colCh;
            obj.BatchOpt.ColorChannel = colCh(obj.mibModel.I{obj.mibModel.Id}.selectedColorChannel);
            obj.Method_Callback();
        end
        ...
        function Method_Callback(obj)
            ...
            obj.BatchOpt.Method = currOperationName;
        ...
        function previewBtn_Callback(obj)
            ...
            colChId = find(ismember(obj.View.handles.ColorChannel.String, obj.BatchOpt.ColorChannel)==1);
            
            img = cell2mat(obj.mibModel.getData2D('image', NaN, NaN, colChId)); %#ok<FNDSB>
            maxInt = obj.mibModel.I{obj.mibModel.Id}.meta('MaxInt');
            ...
            obj.mibModel.setData2D('selection', imgOut);
        ...
        function returnBatchOpt(obj, BatchOptOut)
            ...
            colChList = arrayfun(@(x) sprintf('Ch %d', x), 1:obj.mibModel.I{obj.mibModel.Id}.colors, 'UniformOutput', false);
            
            % add second field to popupmenus with all possible options
            BatchOptOut.ColorChannel{2} = colChList;
            BatchOptOut.Mode{2} = [{'2D, Slice'},{'3D, Stack'}, {'4D, Dataset'}];
            BatchOptOut.Method{2} = [{'Concavity'},{'Entropy'},{'InterMeans iter'},{'InterModes'},...
                {'Mean'},{'Median'},{'MinError'},{'MinError iter'},{'Minimum'},{'Moments'},{'Otsu'},{'Percentile'}];
            BatchOptOut.Destination{2} = [{'selection'},{'mask'}];
            
            BatchOptOut.mibBatchSectionName = 'Menu Tools';
            BatchOptOut.mibBatchActionName = 'Semi-automatic segmentation --> Global thresholding';
        ...
        function continueBtn_Callback(obj)
            ...
            tic
            colChList = arrayfun(@(x) sprintf('Ch %d', x), 1:obj.mibModel.I{obj.mibModel.Id}.colors, 'UniformOutput', false);
            colChId = find(ismember(colChList, obj.BatchOpt.ColorChannel)==1);
            
            switch obj.BatchOpt.Mode{1}
                case '2D, Slice'
                    if ~isfield(obj.BatchOpt, 't')
                        BatchOptLoc.t(1) = obj.mibModel.I{obj.mibModel.Id}.slices{5}(1);
                        BatchOptLoc.t(2) = BatchOptLoc.t(1);
                    end
                    if ~isfield(obj.BatchOpt, 'z')
                        BatchOptLoc.z(1) = obj.mibModel.I{obj.mibModel.Id}.getCurrentSliceNumber();
                        BatchOptLoc.z(2) = BatchOptLoc.z(1);
                    end
                    obj.mibModel.mibDoBackup(BatchOptLoc.Destination{1}, 0);   % backup current data
                case '3D, Stack'
                    if ~isfield(obj.BatchOpt, 't')
                        BatchOptLoc.t(1) = obj.mibModel.I{obj.mibModel.Id}.slices{5}(1);
                        BatchOptLoc.t(2) = BatchOptLoc.t(1);
                    ...
                    if ~isfield(obj.BatchOpt, 'z')
                        BatchOptLoc.z(1) = 1;
                        BatchOptLoc.z(2) = obj.mibModel.I{obj.mibModel.Id}.dim_yxczt(BatchOptLoc.Orientation);
                    end
                    obj.mibModel.mibDoBackup(BatchOptLoc.Destination{1}, 1);   % backup current data
                case '4D, Dataset'
                    if ~isfield(obj.BatchOpt, 't')
                        BatchOptLoc.t(1) = 1;
                        BatchOptLoc.t(2) = obj.mibModel.I{obj.mibModel.Id}.time;
                    end
                    if ~isfield(obj.BatchOpt, 'z')
                        BatchOptLoc.z(1) = 1;
                        BatchOptLoc.z(2) = obj.mibModel.I{obj.mibModel.Id}.dim_yxczt(BatchOptLoc.Orientation);
