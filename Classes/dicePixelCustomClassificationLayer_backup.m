...
classdef dicePixelCustomClassificationLayer < nnet.layer.ClassificationLayer
    ...
    methods
        ...
        function layer = dicePixelCustomClassificationLayer(name, dataDimension)
...
%                 elseif layer.dataDimension == 2.5 %    2.5D network
%                     zVal = ceil(size(T,3)/2);
%                     W = 1 ./ sum(sum(T(:,:,zVal,layer.useClasses,:),1),2).^2;
%                     W(W == Inf) = layer.Epsilon;   % add layer.Epsilon to compensate for missing classes in the patch
% 
%                     intersection = sum(sum(Y(:,:,zVal,layer.useClasses,:).*T(:,:,zVal,layer.useClasses,:),1),2);
%                     union = sum(sum(Y(:,:,zVal,layer.useClasses,:).^2 + T(:,:,zVal,layer.useClasses,:).^2, 1),2);  % union = sum(sum(Y + T, 1),2);        
% 
