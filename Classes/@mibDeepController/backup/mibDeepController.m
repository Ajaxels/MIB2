classdef mibDeepController < handle
    ...
    properties
        ...
        Aug2DFuncProbability    % if obj.View.handles.newaugmentationsystemCheckBox.Value == 1
    ...
    methods
        ...
        function status = setAug2DFuncHandles(obj, inputPatchSize)
            ...
            if obj.View.handles.newaugmentationsystemCheckBox.Value == 1
                ...
                if obj.AugOpt2D.RandYReflection(1) == 1
                ...
                if obj.AugOpt2D.Rotation90(1) == 1
                ...
                if obj.AugOpt2D.ReflectedRotation90(1) == 1
                ...
                % random rotation
                if obj.AugOpt2D.RandRotation(1) ~= 0 || obj.AugOpt2D.RandRotation(2) ~= 0
                ...
                % random scale
                if obj.AugOpt2D.RandScale(1) ~= 1 || obj.AugOpt2D.RandScale(2) ~= 1
                ...
                % random x scale
                if obj.AugOpt2D.RandXScale(1) ~= 1 || obj.AugOpt2D.RandXScale(2) ~= 1
                ...
                % random y scale
                if obj.AugOpt2D.RandYScale(1) ~= 1 || obj.AugOpt2D.RandYScale(2) ~= 1
                ...
                % random xshear
                if obj.AugOpt2D.RandXShear(1) ~= 0 || obj.AugOpt2D.RandXShear(2) ~= 0
                ...
                % random yshear
                if obj.AugOpt2D.RandYShear(1) ~= 0 || obj.AugOpt2D.RandYShear(2) ~= 0
                ...
                if obj.AugOpt2D.GaussianNoise(1) ~= 0 || obj.AugOpt2D.GaussianNoise(2) ~= 0
                ...
                % random Hue jitter
                if obj.AugOpt2D.HueJitter(1) ~= 0 || obj.AugOpt2D.HueJitter(2) ~= 0 && inputPatchSize(4) == 3
                ...
                if obj.AugOpt2D.SaturationJitter(1) ~= 0 || obj.AugOpt2D.SaturationJitter(2) ~= 0 && inputPatchSize(4) == 3
                ...
                if obj.AugOpt2D.BrightnessJitter(1) ~= 0 || obj.AugOpt2D.BrightnessJitter(2) ~= 0
                ...
                if obj.AugOpt2D.ContrastJitter(1) ~= 1 || obj.AugOpt2D.ContrastJitter(2) ~= 1
                ...
                if obj.AugOpt2D.ImageBlur(1) ~= 0 || obj.AugOpt2D.ImageBlur(2) ~= 0
                ...
                if obj.AugOpt2D.RandXReflection
                    obj.Aug2DFuncNames = [obj.Aug2DFuncNames, {'RandXReflection'}];
                end

                % Y-reflections, true or false
                if obj.AugOpt2D.RandYReflection
                    obj.Aug2DFuncNames = [obj.Aug2DFuncNames, {'RandYReflection'}];
                end

                % 90 and 270 degree rotations
                if obj.AugOpt2D.Rotation90
                    obj.Aug2DFuncNames = [obj.Aug2DFuncNames, {'Rotation90'}];
                end

                % reflected rotations
                if obj.AugOpt2D.ReflectedRotation90
                    obj.Aug2DFuncNames = [obj.Aug2DFuncNames, {'ReflectedRotation90'}];
                end
            ...
            if isempty(obj.Aug2DFuncNames)
                uialert(obj.View.gui, ...
                    sprintf('!!! Error !!!\n\nAugmentation types were not selected!\nPlease use set 2D augmentation settings dialog'), ...
                    'Wrong augmentations');
                return;
            end
            status = 1;
        ...
        function [patchOut, augList, augPars] = augmentAndCrop2dPatch(obj, patchIn, inputPatchSize, outputPatchSize, mode)
            ...
            if strcmp(mode, 'show')
                ...
                if obj.View.handles.newaugmentationsystemCheckBox.Value == 1
                    augPars = nan([size(patchIn,1), 17]);
                else
                    augPars = nan([size(patchIn,1), 2]);
                end
                    ...
                    if strcmp(mode, 'crop')  % do only crop
                        ...
                        if obj.View.handles.newaugmentationsystemCheckBox.Value == 1
                            ...
                            if rndIdx > obj.AugOpt2D.Fraction   % if index lower than obj.AugOpt2D.Fraction -> augment the data
                                ...
                                augList{id} = obj.Aug2DFuncNames{augFuncIndex};

                                switch obj.Aug2DFuncNames{augFuncIndex}
                                    case 'RandXReflection'
                                    ...
                                    case 'RandRotation'
                                        augPars(id,1) = obj.AugOpt2D.RandRotation(1) + (obj.AugOpt2D.RandRotation(2)-obj.AugOpt2D.RandRotation(1))*rand;
                                        tform = randomAffine2d('Rotation', [augPars(id,1) augPars(id,1)]);
                                        outputView = affineOutputView([size(patchIn.InputImage{id},1) size(patchIn.InputImage{id},2)], tform);
                                        out = imwarp(patchIn.InputImage{id}, tform, 'cubic', 'OutputView', outputView, 'FillValues', obj.AugOpt2D.FillValue);
                                        respOut = imwarp(patchIn.(fieldId){id}, tform, 'nearest', 'OutputView', outputView);
                                    case 'RandScale'
                                        augPars(id,1) = obj.AugOpt2D.RandScale(1) + (obj.AugOpt2D.RandScale(2)-obj.AugOpt2D.RandScale(1))*rand;
                                        tform = randomAffine2d('Scale', [augPars(id,1) augPars(id,1)]);
                                        outputView = affineOutputView([size(patchIn.InputImage{id},1) size(patchIn.InputImage{id},2)], tform);
                                        out = imwarp(patchIn.InputImage{id}, tform, 'cubic', 'OutputView', outputView, 'FillValues', obj.AugOpt2D.FillValue);
                                        respOut = imwarp(patchIn.(fieldId){id}, tform, 'nearest', 'OutputView', outputView);
                                    case 'RandXScale'
                                        augPars(id,1) = obj.AugOpt2D.RandXScale(1) + (obj.AugOpt2D.RandXScale(2)-obj.AugOpt2D.RandXScale(1))*rand;
                                        T = [augPars(id,1) 0 0; 0 1 0; 0 0 1];
                                        tform = affine2d(T);
                                        outputView = affineOutputView([size(patchIn.InputImage{id},1) size(patchIn.InputImage{id},2)], tform);
                                        out = imwarp(patchIn.InputImage{id}, tform, 'cubic', 'OutputView', outputView, 'FillValues', obj.AugOpt2D.FillValue);
                                        respOut = imwarp(patchIn.(fieldId){id}, tform, 'nearest', 'OutputView', outputView);
                                    case 'RandYScale'
                                        augPars(id,1) = obj.AugOpt2D.RandYScale(1) + (obj.AugOpt2D.RandYScale(2)-obj.AugOpt2D.RandYScale(1))*rand;
                                        T = [1 0 0; 0 augPars(id,1) 0; 0 0 1];
                                        tform = affine2d(T);
                                        outputView = affineOutputView([size(patchIn.InputImage{id},1) size(patchIn.InputImage{id},2)], tform);
                                        out = imwarp(patchIn.InputImage{id}, tform, 'cubic', 'OutputView', outputView, 'FillValues', obj.AugOpt2D.FillValue);
                                        respOut = imwarp(patchIn.(fieldId){id}, tform, 'nearest', 'OutputView', outputView);
                                    case 'RandXShear'
                                        augPars(id,1) = obj.AugOpt2D.RandXShear(1) + (obj.AugOpt2D.RandXShear(2)-obj.AugOpt2D.RandXShear(1))*rand;
                                        tform = randomAffine2d('XShear', [augPars(id,1) augPars(id,1)]);
                                        outputView = affineOutputView([size(patchIn.InputImage{id},1) size(patchIn.InputImage{id},2)], tform);
                                        out = imwarp(patchIn.InputImage{id}, tform, 'cubic', 'OutputView', outputView, 'FillValues', obj.AugOpt2D.FillValue);
                                        respOut = imwarp(patchIn.(fieldId){id}, tform, 'nearest', 'OutputView', outputView);
                                    case 'RandYShear'
                                        augPars(id,1) = obj.AugOpt2D.RandYShear(1) + (obj.AugOpt2D.RandYShear(2)-obj.AugOpt2D.RandYShear(1))*rand;
                                        tform = randomAffine2d('YShear', [augPars(id,1) augPars(id,1)]);
                                        outputView = affineOutputView([size(patchIn.InputImage{id},1) size(patchIn.InputImage{id},2)], tform);
                                        out = imwarp(patchIn.InputImage{id}, tform, 'cubic', 'OutputView', outputView, 'FillValues', obj.AugOpt2D.FillValue);
                                        respOut = imwarp(patchIn.(fieldId){id}, tform, 'nearest', 'OutputView', outputView);
                                    case 'HueJitter'
                                        if size(patchIn.InputImage{id}, 3) == 3
                                            augPars(id,1) = obj.AugOpt2D.HueJitter(1) + (obj.AugOpt2D.HueJitter(2)-obj.AugOpt2D.HueJitter(1))*rand;
                                            out = jitterColorHSV(patchIn.InputImage{id}, 'Hue', [augPars(id,1) augPars(id,1)]);
                                            % add Saturation jitter with 50% chance
                                            if ismember('SaturationJitter', obj.Aug2DFuncNames) && rand()<0.5
                                                augPars(id,2) = obj.AugOpt2D.SaturationJitter(1) + (obj.AugOpt2D.SaturationJitter(2)-obj.AugOpt2D.SaturationJitter(1))*rand;
                                                out = jitterColorHSV(out, 'Saturation', [augPars(id,2) augPars(id,2)]);
                                                augList{id} = [augList{id} '+Sat'];
                                            end
                                        else
                                            out = patchIn.InputImage{id};
                                        end
                                    ...
                                    case 'SaturationJitter'
                                        if size(patchIn.InputImage{id}, 3) == 3
                                            augPars(id,1) = obj.AugOpt2D.SaturationJitter(1) + (obj.AugOpt2D.SaturationJitter(2)-obj.AugOpt2D.SaturationJitter(1))*rand;
                                            out = jitterColorHSV(patchIn.InputImage{id}, 'Saturation', [augPars(id,1) augPars(id,1)]);
                                            % add Hue jitter with 50% chance
                                            if ismember('HueJitter', obj.Aug2DFuncNames) && rand()<0.5
                                                augPars(id,2) = obj.AugOpt2D.HueJitter(1) + (obj.AugOpt2D.HueJitter(2)-obj.AugOpt2D.HueJitter(1))*rand;
                                                out = jitterColorHSV(out, 'Hue', [augPars(id,2) augPars(id,2)]); 
                                                augList{id} = [augList{id} '+Hue'];
                                            end
                                        else
                                            out = patchIn.InputImage{id};
                                        end
                                        respOut = patchIn.(fieldId){id};
                                    case 'BrightnessJitter'
                                        augPars(id,1) = obj.AugOpt2D.BrightnessJitter(1) + (obj.AugOpt2D.BrightnessJitter(2)-obj.AugOpt2D.BrightnessJitter(1))*rand;
                                        if size(patchIn.InputImage{id}, 3) == 3
                                            out = jitterColorHSV(patchIn.InputImage{id}, 'Brightness', [augPars(id,1) augPars(id,1)]);
                                        else
                                            out = patchIn.InputImage{id} + augPars(id,1)*255;
                                        end
                                        respOut = patchIn.(fieldId){id};
                                    case 'ContrastJitter'
                                        augPars(id,1) = obj.AugOpt2D.ContrastJitter(1) + (obj.AugOpt2D.ContrastJitter(2)-obj.AugOpt2D.ContrastJitter(1))*rand;
                                        if size(patchIn.InputImage{id}, 3) == 3
                                            out = jitterColorHSV(patchIn.InputImage{id}, 'Contrast', [augPars(id,1) augPars(id,1)]);
                                        else
                                            out = patchIn.InputImage{id}.*augPars(id,1);
                                        end
                                        respOut = patchIn.(fieldId){id};
                                    case 'ImageBlur'
                                        augPars(id,1) = obj.AugOpt2D.ImageBlur(1) + (obj.AugOpt2D.ImageBlur(2)-obj.AugOpt2D.ImageBlur(1))*rand;
                                        out =  imgaussfilt(patchIn.InputImage{id}, augPars(id,1));
                                        respOut = patchIn.(fieldId){id};
                                    case 'GaussianNoise'
                                        augPars(id,1) = obj.AugOpt2D.GaussianNoise(1) + (obj.AugOpt2D.GaussianNoise(2)-obj.AugOpt2D.GaussianNoise(1))*rand;
                                        out =  imnoise(patchIn.InputImage{id}, 'gaussian', 0, augPars(id,1));
                                        respOut = patchIn.(fieldId){id};
                                    case 'PoissonNoise'
                                        out =  imnoise(patchIn.InputImage{id}, 'poisson');
                                        respOut = patchIn.(fieldId){id};
                                end
                            end
                            %                     if obj.BatchOpt.O_PreviewImagePatches && rand < obj.BatchOpt.O_FractionOfPreviewPatches{1}
                            %                         if isfield(obj.TrainingProgress, 'hFig')
                            %                             if size(out, 3) == 3 || size(out, 3) == 1
                            %                                 image(obj.TrainingProgress.imgPatch, out);
                            %                             elseif size(out, 3) == 2
                            %                                 out2 = out;
                            %                                 out2(:,:,3) = zeros([size(out,1) size(out,2)]);
                            %                                 image(obj.TrainingProgress.imgPatch, out2);
                            %                             end
                            %                             image(obj.TrainingProgress.labelPatch, uint8(respOut), 'CDataMapping', 'scaled');
                            %                             drawnow;
                            %                         end
                            %                         %figure(112345)
        ...
        function [patchOut, augList, augPars] = augmentAndCrop3dPatch(obj, patchIn, inputPatchSize, outputPatchSize, mode)
            ...
            if obj.BatchOpt.O_PreviewImagePatches && rand < obj.BatchOpt.O_FractionOfPreviewPatches{1}
                if isfield(obj.TrainingProgress, 'UIFigure')
                    ...
                    imagesc(obj.TrainingProgress.labelPatch, uint8(inpResponse{1}(:,:,zVal)), [0 obj.BatchOpt.T_NumberOfClasses{1}]);
                    drawnow;
                end
                %figure(112345)
                %imshowpair(out, uint8(respOut), 'montage');
        ...
        function disable2Daugmentation(obj)
            ...
            obj.AugOpt2D.Fraction = .9;
            obj.AugOpt2D.FillValue = 255;
            obj.AugOpt2D.RandXReflection = false;
            obj.AugOpt2D.RandYReflection = false;
            obj.AugOpt2D.Rotation90 = false;
            obj.AugOpt2D.ReflectedRotation90 = false;
            obj.AugOpt2D.RandRotation = [0 0];
            obj.AugOpt2D.RandXScale = [1 1];
            obj.AugOpt2D.RandYScale = [1 1];
            obj.AugOpt2D.RandScale = [1 1];
            obj.AugOpt2D.RandXShear = [0 0];
            obj.AugOpt2D.RandYShear = [0 0];
            obj.AugOpt2D.HueJitter = [0 0];
            obj.AugOpt2D.SaturationJitter = [0 0];
            obj.AugOpt2D.BrightnessJitter = [0 0];
            obj.AugOpt2D.ContrastJitter = [1 1];
            obj.AugOpt2D.GaussianNoise = [0 0]; % variance
            obj.AugOpt2D.PoissonNoise = false;
            obj.AugOpt2D.ImageBlur = [0 0];
        ...
        function setAugmentation2DSettings(obj)
            ...
            if obj.View.handles.newaugmentationsystemCheckBox.Value == 1
                ...
                options.Title = 'Each augmentation is defined with 2 or 3 values, the first values define variation of the augmentation filter, while the last value specify probability of triggering the augmentation. The resulting augmented patch may be made from a cocktail of many augmentations'; 
        ...
        function StartPrediction2D(obj)
            ...
            while hasdata(imgDS)
                ...
                if strcmp(obj.BatchOpt.T_ConvolutionPadding{1}, 'same')     % same
                    ...
                    padSizePre  = (inputPatchSize(1:2)-outputPatchSize(1:2))/2;
        ...
        function EvaluateSegmentation(obj)
            ...
            % add name of files
            ssmStruct.ImageMetrics.ModelFilename = {predictionList.name}';
            ssmStruct.ImageMetrics.GroundTruthFilename = {truthList.name}';
            ...
            if answer{3}    % save in Excel format
                ...
                excelHeader{5,1} = 'Model filename';  excelHeader{5,2} = 'Ground truth filename';
                
                excelHeader(6:6+numel(truthList)-1, 1) = {predictionList.name}';
                excelHeader(6:6+numel(truthList)-1, 2) = {truthList.name}';
                writecell(excelHeader, fn, 'FileType', 'spreadsheet', 'Sheet', 'ImageMetrics', 'Range', 'A1');
                %writetable(ssm.ImageMetrics, fn, 'FileType', 'spreadsheet', 'Sheet', 'ImageMetrics', 'WriteRowNames', true, 'Range', 'C5');
                writetable(ssmStruct.ImageMetrics, fn, 'FileType', 'spreadsheet', 'Sheet', 'ImageMetrics', 'WriteRowNames', true, 'Range', 'C5');
            ...
            if obj.PatchPreviewOpt.labelShow
                for z=1:obj.PatchPreviewOpt.noImages
                    if obj.View.handles.newaugmentationsystemCheckBox.Value == 1
                        ...
                        if isnan(augParameter(z,1))
                            textString = augOperation{z};
                        elseif isnan(augParameter(z,2))
                            textString = sprintf('%s\n%.3f', augOperation{z}, augParameter(z,1));
                        else
                            textString = sprintf('%s\n%.3f %.3f', augOperation{z}, augParameter(z,1), augParameter(z,2));
                        end
                        Iout{z} = insertText(Iout{z}, [1 1], ...
                            textString, 'FontSize', obj.PatchPreviewOpt.labelSize, ...
                            'TextColor', obj.PatchPreviewOpt.labelColor, ...
